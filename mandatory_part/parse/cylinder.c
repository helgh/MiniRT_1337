...
/*   Updated: 2025/03/20 00:32:57 by hael-ghd         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	cylinder_compenent(t_scene *scene, t_cylinder *cy)
{
	t_tmp_heap	*tmp;
...
void	parse_cylinder(t_scene *scene, char **line)
{
	...
	len = lengh(line);
	if (len != 6 || valid_float(line[3], false) || valid_float(line[4], false))
	...
	cylinder_compenent(scene, cylinder);
	if (!scene->cylinder)
